...
const base={name:' Jan Novák ',phone:'+420 777 123 456',email:'JAN.NOVAK@EXAMPLE.CZ',place:' Uherské Hradiště ',note:' Prosím zavolat. ',fenceType:'Panelový plot',height:153,segments:[{name:'Předek',length:20,connection:'začátek'},{name:'Bok',length:17,connection:'navazuje rohem'}],options:['3D','Zelená'],gate:false,gateType:'double',gateWidth:4,gateDrive:'none',gateSection:0,gatePos:0,wicket:false,wicketWidth:1,wicketSection:0,wicketPos:0,scope:'Materiál',displayedPrice:'42 000 Kč',priceKind:'ověřená cena',priceReason:'',placeFromCalculator:'Uherské Hradiště'};
...
ok(d.schemaVersion===1,'lead payload schema version must stay explicit');
...
ok(v.valid===false&&v.errors.some(e=>e.code==='segments'),'lead must contain at least one positive fence segment');
v=core.validateLead({...base,priceKind:'neplatné zadání'});
ok(v.valid===false&&v.errors.some(e=>e.code==='invalid-price'),'invalid calculator state must block lead preparation');
...
ok(v.valid===false&&v.errors.some(e=>e.code==='gate-placement'||e.code==='gate-section'),'out-of-range gate section must not create a silently valid lead');
...
ok(out.includes('+420777123456')&&out.includes('Panelový plot')&&out.includes('Brána: double · 4 m · auto · úsek 1 · pozice 8 m'),'clipboard export must use the same normalized payload as future backend submission');
ok(out.includes('Předek 20 m')&&out.includes('Bok 17 m · navazuje rohem'),'clipboard export must preserve all normalized fence sections');

if(fail.length){console.error('Lead scenario checks failed:\n- '+fail.join('\n- '));process.exit(1)}
console.log('Lead scenario checks OK: contact normalization, geometry validation and export schema are protected');
