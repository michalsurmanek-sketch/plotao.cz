...
const src=fs.readFileSync('assets/ui-bootstrap-v1.js','utf8'),calculator=fs.readFileSync('assets/calculator-v3.js','utf8'),leadSafety=fs.readFileSync('assets/lead-safety-v1.js','utf8'),lead=fs.readFileSync('assets/lead-core-v1.js','utf8'),server=fs.readFileSync('supabase/functions/submit-lead/validation.mjs','utf8'),fail=[];const ok=(v,m)=>{if(!v)fail.push(m)};
...
ok(src.includes('data-segment-name value="')&&src.includes('maxlength="100" aria-label="Název úseku"'),'segment editor must mark the name field explicitly and enforce the 100-character backend limit');
ok(src.includes("const name=row.querySelector('[data-segment-name]')"),'segment editor must bind name edits through the explicit segment-name selector');
ok(calculator.includes("name:el.querySelector('[data-segment-name]')?.value?.trim()"),'calculator geometry/placement must read only the explicit segment-name field');
ok(leadSafety.includes("name:e.querySelector('[data-segment-name]')?.value?.trim()"),'lead snapshot must read only the explicit segment-name field');
ok(!src.includes("querySelector('input:not([type=number])')")&&!calculator.includes("querySelector('input:not([type=number])')")&&!leadSafety.includes("querySelector('input:not([type=number])')"),'segment name readers must never fall back to a selector that can accidentally match the connection checkbox');
...
console.log('Segment edit state checks OK: explicit bounded names, raw lengths and keyboard focus survive segment rerenders');
