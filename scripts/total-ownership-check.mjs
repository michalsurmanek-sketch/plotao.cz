...
ok(price.includes("const blocked=coreBlocked(),partial=hasPartialData(),sc=scope(),totalAllowed=sc==='material'&&!blocked&&!window.PLOTAO_ACCURACY_BLOCK"),'generic total permission must require material scope, supported construction and released accuracy state');
ok(price.includes("if(totalAllowed)setMain")&&price.includes("else{clearOwnedTotal();clearBenchmarkNote();return}"),'blocked or non-material generic pricing must clear both its old total and total-level benchmark badge');
