...
ok(/<script src="\/assets\/ui-bootstrap-v1\.js(?:\?[^\"]+)?"><\/script>/.test(index),'index must load the external UI bootstrap');
...
ok(ui.includes("function reduceMotion(){return!!window.matchMedia?.('(prefers-reduced-motion: reduce)').matches}")&&ui.includes("behavior:reduceMotion()?'auto':'smooth'"),'result navigation must avoid smooth scrolling when the user prefers reduced motion');
ok(ui.includes("scope.setAttribute('role','group')")&&ui.includes("scope.setAttribute('aria-label','Rozsah kalkulace')")&&ui.includes("document.querySelectorAll('#scope button').forEach")&&ui.includes("x.setAttribute('aria-pressed',on?'true':'false')")&&ui.includes("scope.addEventListener('click',e=>{const b=e.target.closest('button');if(b)syncScope(b)},true)"),'result scope controls must collect every button, expose their group name, and capture aria-pressed synchronization before later handlers');
