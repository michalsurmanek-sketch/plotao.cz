...
ok(ui.includes("scope.setAttribute('role','group')")&&ui.includes("scope.setAttribute('aria-label','Rozsah kalkulace')")&&ui.includes("x.setAttribute('aria-pressed',on?'true':'false')")&&ui.includes("b.addEventListener('click',()=>syncScope(b))"),'result scope controls must expose their group name and synchronize aria-pressed directly after each selection');
