...
ok(ui.includes("scope.setAttribute('role','group')")&&ui.includes("scope.setAttribute('aria-label','Rozsah kalkulace')")&&ui.includes("x.setAttribute('aria-pressed',on?'true':'false')")&&ui.includes("scope.addEventListener('click',e=>{const b=e.target.closest('button');if(b)syncScope(b)},true)"),'result scope controls must expose their group name and capture aria-pressed synchronization before later handlers');
