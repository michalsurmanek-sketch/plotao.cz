...
let r=core.prepare(payload,{enabled:true,endpoint:'https://api.example.cz/leads',allowedOrigins:['https://api.example.cz']},'2026-09-12T20:30:00.000Z');
ok(r.ok===true&&r.endpoint==='https://api.example.cz/leads','explicitly enabled approved endpoint must prepare a request');
...
r=core.prepare(payload,{enabled:false,endpoint:'https://api.example.cz/leads',allowedOrigins:['https://api.example.cz']});
ok(r.ok===false&&r.code==='disabled','endpoint alone must never activate delivery');
r=core.prepare(payload,{enabled:true,endpoint:'https://evil.example/leads',allowedOrigins:['https://api.example.cz']});
ok(r.ok===false&&r.code==='origin','endpoint outside the explicit allowlist must be blocked');
r=core.prepare(payload,{});
ok(r.ok===false&&r.code==='disabled','missing runtime config must keep server delivery disabled');
r=core.prepare({...payload,schemaVersion:1},{enabled:true,endpoint:'https://api.example.cz/leads',allowedOrigins:['https://api.example.cz']});
...
console.log('Lead transport scenario checks OK: explicit enable flag, HTTPS allowlist, disabled-by-default config, script order, timeout and fallback are protected');
