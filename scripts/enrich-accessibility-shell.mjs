...
const style='<style data-plotao-skip-style="1">.plotao-skip-link{position:fixed;left:16px;top:16px;z-index:1000;padding:10px 14px;border-radius:10px;background:#fff;color:#163d2d;font-weight:800;text-decoration:none;box-shadow:0 8px 24px #0002;transform:translateY(-220%);transition:transform .15s ease}.plotao-skip-link:focus,.plotao-skip-link:focus-visible{transform:none;outline:3px solid #f07828;outline-offset:3px}@media(prefers-reduced-motion:reduce){.plotao-skip-link{transition:none}}</style>';
...
if(skipCount!==1||styleCount!==1||!/<main\b[^>]*id="kalkulator"[^>]*tabindex="-1"/i.test(html)){
  throw new Error(`Accessibility shell enrichment failed: skip=${skipCount} style=${styleCount} focusableMain=${/<main\b[^>]*id="kalkulator"[^>]*tabindex="-1"/i.test(html)}`);
}

fs.writeFileSync(file,html,'utf8');
console.log('Accessibility shell OK: keyboard skip link targets focusable #kalkulator without changing the default visual layout');
