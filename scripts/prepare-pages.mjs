...
function replaceRequired(name,from,to){
  if(html.includes(from)){html=html.replace(from,to);changes.push(name);return;}
  if(html.includes(to)){changes.push(`${name} (already migrated)`);return;}
  throw new Error(`Pages build transform missing both legacy and migrated source: ${name}`);
}
function neutralizeLegacy(name,from,to){
  if(html.includes(from)){html=html.replace(from,to);changes.push(name);return;}
  // Legacy pricing is intentionally allowed to disappear from index.html permanently.
  changes.push(`${name} (removed from source)`);
}
...
for(const [oldPrice,label] of [[1680,'panel'],[720,'mesh'],[2650,'concrete'],[3180,'privacy'],[4650,'aluminium'],[3950,'gabion'],[3850,'metal'],[4250,'masonry'],[780,'mobile'],[2900,'other']]){
  neutralizeLegacy(`legacy ${label} base price`,`price:${oldPrice}`,'price:0');
}

const legacyNeutralizations=[
...
for(const [name,from,to] of legacyNeutralizations) neutralizeLegacy(name,from,to);
...
for(const [name,from,to] of sourceMigrations) replaceRequired(name,from,to);
...
console.log(`Pages build prepared: ${changes.length} transforms, ${activeScripts.length} modules, SHA ${sha}`);
