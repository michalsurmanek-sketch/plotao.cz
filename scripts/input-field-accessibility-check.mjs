...
ok(src.includes("if(s?.code==='segment-length')return $$('#segmentList input[type=number]').find(x=>+(x.value||0)<=0)||null"),'shared validity must route zero/empty segment length to the actual invalid length input');
...
console.log('Input field accessibility checks OK: height, segment length and total-length errors mark actionable controls');
