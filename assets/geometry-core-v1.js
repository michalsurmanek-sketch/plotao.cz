...
function solveGeometry(input={}){
  ...
  const key=(i,p)=>near(p,0)?(linked(i)?'joint:'+(i-1):'outer:'+i+':start'):near(p,segments[i]?.len||0)?(i<segments.length-1&&linked(i+1)?'joint:'+i:'outer:'+i+':end'):'s:'+i+':'+p.toFixed(4);
  ...
  for(let i=0;i<segments.length;i++){if(i===0||!linked(i))add('outer:'+i+':start','end');if(i===segments.length-1||!linked(i+1))add('outer:'+i+':end','end');if(i<segments.length-1&&linked(i+1))add('joint:'+i,'corner')}
  ...
  for(const s of segments){const os=openings.filter(o=>o.s===s.i).sort((a,b)=>a.p-b.p);let cursor=0,rr=[];for(const o of os){if(o.p>cursor+.001)rr.push([cursor,o.p]);cursor=Math.max(cursor,o.p+o.w)}if(cursor<s.len-.001)rr.push([cursor,s.len]);for(const o of os){let sides=(rr.some(r=>near(r[1],o.p))?1:0)+(rr.some(r=>near(r[0],o.p+o.w))?1:0);if(near(o.p,0)&&linked(s.i)&&endpointHasFence(s.i-1,'end'))sides++;if(near(o.p+o.w,s.len)&&s.i<segments.length-1&&linked(s.i+1)&&endpointHasFence(s.i+1,'start'))sides++;if(o.kind==='gate')gateSides+=sides;else wicketSides+=sides}
  ...
  const c={line:0,strain:0,end:0,corner:0,gate:0,wicket:0};for(const r of nodes.values())c[r]++;const total=Object.values(c).reduce((a,b)=>a+b,0),corners=segments.slice(1).filter(s=>s.connected).length,gross=segments.reduce((a,s)=>a+s.len,0);
  return{...c,total,fields,fenceLen,gross,gap,runs,gateSides,wicketSides,corners,segments};
}
