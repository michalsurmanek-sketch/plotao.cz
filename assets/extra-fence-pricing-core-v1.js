(function(root,factory){
  ...
  const mobile={panelLength:3.5,panelHeight:1.9,panelUnit:1286,footUnit:176,connectorUnit:78,solid:{panelLength:2.8,panelHeight:2,panelUnit:2536.16,footUnit:176,connectorUnit:78}};
  ...
  function computeExtraFencePrice(input={}){
    ...
    if(type==='mobile'){
      if(!['mesh','solid'].includes(config.variant))return{unsupported:true,kind:'mobile',reason:'variant',length,height};
      const spec=config.variant==='solid'?mobile.solid:mobile;
      ...
      const counts=runs.map(x=>Math.ceil(x/spec.panelLength)),panels=counts.reduce((a,x)=>a+x,0),feet=counts.reduce((a,x)=>a+(x?x+1:0),0),connectors=counts.reduce((a,x)=>a+Math.max(0,x-1),0),panelCost=panels*spec.panelUnit,feetCost=feet*spec.footUnit,connectorCost=connectors*spec.connectorUnit,materialTotal=panelCost+feetCost+connectorCost;
      return{unsupported:false,kind:config.variant==='solid'?'mobile-solid':'mobile-mesh',materialTotal,panels,feet,connectors,panelCost,feetCost,connectorCost,counts,runs,length,height,panelHeight:spec.panelHeight,panelLength:spec.panelLength,panelUnit:spec.panelUnit,footUnit:spec.footUnit,connectorUnit:spec.connectorUnit};
    }
