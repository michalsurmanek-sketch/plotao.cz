...
const selected=hedgeVariants[s.variant],base='/assets/fence-types/hedge-'+s.variant+'-',labels=['Detail listů',s.variant==='mixed'?'Květy a barvy':'Tvarování plotu','Výsadba a závlaha',s.variant==='deciduous'?'Podzimní vzhled':'Plot v zahradě'],gallery='<div class="mobile-thumbs">'+labels.map((label,i)=>'<figure><img src="'+base+(i+1)+'.webp" alt="'+selected.title+' živý plot – '+label+'" loading="lazy" decoding="async" width="600" height="400"><figcaption>'+label+'</figcaption></figure>').join('')+'</div>';
...
return '<div class="mobile-config-title"><span class="mobile-config-icon" aria-hidden="true">'+gearIcon()+'</span><div><h3>Konfigurace živého plotu</h3><p>Vyberte charakter výsadby pro vaši zahradu</p></div></div><div class="mobile-choices" role="group" aria-label="Varianta živého plotu">'+choices+'</div><div class="mobile-product"><div class="mobile-product-head"><strong>Vybraná varianta</strong><span>Individuální nabídka</span></div><div class="mobile-product-main"><img src="'+base+'0.webp" alt="'+selected.title+' živý plot – AI vizualizace" decoding="async" width="1200" height="800"><div><h3>'+selected.title+' živý plot</h3><p>'+selected.description+'</p><p>Konkrétní druhy rostlin, jejich velikost při výsadbě a rozestupy upřesníme v nabídce.</p></div></div>'+gallery+tip('Délku výsadby a požadovanou cílovou výšku zadejte v rozměrech plotu. Cena závisí na vybraných rostlinách, jejich počtu, přípravě půdy a rozsahu výsadby. Obrázky jsou ilustrační AI vizualizace.')+'</div>';
}
function render(){
const b=ensure();
if(!b)return;
